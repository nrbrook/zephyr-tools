...
def prepare_treemap_data(objects, by_symbol=False, get_section_total=None, get_object_total=None, group_by_directory=None):
    """
    ...
    if by_symbol:
        ...
        return [{
            'type': 'treemap',
            'labels': ['Total'] + [f"{symbol} ({size:,d} bytes)" for symbol, size in sorted(symbol_sizes.items(), key=lambda x: x[1], reverse=True)],
            'parents': [''] + ['Total'] * len(symbol_sizes),
            'values': [sum(symbol_sizes.values())] + list(dict(sorted(symbol_sizes.items(), key=lambda x: x[1], reverse=True)).values()),
            'textinfo': 'label',
            'hoverongaps': False,
            'maxdepth': 2,
            'branchvalues': 'total'
        ...
        for dirname, dir_objects in sorted(grouped.items(), key=lambda x: sum(get_object_total(sections) for sections in x[1].values()), reverse=True):
...
def generate_html_report(objects, mode=None, by_symbol=False, output_file=None, get_section_total=None, get_object_total=None, group_by_directory=None):
    """
    ...
    template_data = {
        ...
        "treemap_data": json.dumps(prepare_treemap_data(
            objects, 
            by_symbol=by_symbol,
    ...
    if by_symbol:
        ...
        for dirname, dir_objects in sorted(grouped.items(), 
                                        key=lambda x: sum(get_object_total(sections) for sections in x[1].values()),
                                        reverse=True):
            dir_data = {
                "name": dirname,
